...
- **Relay transport shape decision**: Added `docs/superpowers/plans/2026-07-02-ra-pwa-relay-transport-shape-decision.md` and `npm run check:pwa-relay-transport-decision`. The decision selects WebSocket as the first relay prototype substrate, keeps HTTP polling as fallback/diagnostics, and keeps `live-loopback` as the product default until relay auth/session/deployment/UX evidence exists.
- **Relay WebSocket auth/session contract**: Added `docs/superpowers/plans/2026-07-02-ra-pwa-relay-websocket-auth-session.md` plus Rust/PWA helpers for WebSocket session tickets and daemon/companion connect messages. The contract binds session token, session id, daemon pubkey, companion device/key identity, and expiry, while keeping actual bridge enforcement and signed relay tickets as follow-up work.
