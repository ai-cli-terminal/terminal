...
|---|---|---|---|
| P1 local | Managed relay control-plane contract | Define owner/responsibility boundaries, tenant/session isolation requirements, operator-visible state, and audit boundaries before managed runtime implementation | Managed relay operations planning is complete; managed relay remains deferred |
...
evidence, runbook closeout, managed operations planning도 완료됐다.
가장 높은 가치의 다음 release 작업은 외부 환경에서 runbook을 실행하는
**Windows MSI 재검토**와 **Android signing/buildserver evidence**다. 현재 개발 host에서
바로 진행 가능한 다음 로컬 작업은 **Managed relay control-plane contract**다.
