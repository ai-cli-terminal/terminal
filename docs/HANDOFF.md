...
`docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-tenant-aggregate-usage-export-smoke.md`다. Relay/M2는
...
active session and byte quota smoke와 tenant aggregate usage export smoke까지
닫혔지만 implementationCanStart=false다. explicit self-hosted relay readiness는
green이며 다음 로컬 작업은 Managed relay support redaction and access review
evidence다.
