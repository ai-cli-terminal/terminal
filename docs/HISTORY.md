...
- **PWA relay HTTP bridge smoke**: Added `docs/superpowers/plans/2026-07-02-ra-pwa-relay-http-bridge-smoke.md` and `npm run smoke:pwa-relay-http-bridge`. The smoke runs a local dependency-free HTTP bridge, posts relay frame JSON from the browser PWA module, verifies duplicate sequence rejection and expired-frame dropping, and keeps payload decode at endpoint helpers.
- **PWA relay WebSocket bridge smoke**: Added `docs/superpowers/plans/2026-07-02-ra-pwa-relay-websocket-bridge-smoke.md` and `npm run smoke:pwa-relay-websocket-bridge`. The smoke runs a dependency-free local WebSocket bridge, connects browser PWA daemon/companion peers, verifies full-duplex relay frame routing, duplicate sequence rejection, and expired-frame dropping while keeping `payload_json` opaque to the bridge.
- **Relay transport shape decision**: Added `docs/superpowers/plans/2026-07-02-ra-pwa-relay-transport-shape-decision.md` and `npm run check:pwa-relay-transport-decision`. The decision selects WebSocket as the first relay prototype substrate, keeps HTTP polling as fallback/diagnostics, and keeps `live-loopback` as the product default until hosted deployment, daemon integration, and UX evidence exists.
- **Relay WebSocket auth/session contract**: Added `docs/superpowers/plans/2026-07-02-ra-pwa-relay-websocket-auth-session.md` plus Rust/PWA helpers for WebSocket session tickets and daemon/companion connect messages. The contract binds session token, session id, daemon pubkey, companion device/key identity, and expiry.
- **Relay WebSocket bridge auth enforcement**: Added `docs/superpowers/plans/2026-07-02-ra-pwa-relay-websocket-bridge-auth.md` and tightened `npm run smoke:pwa-relay-websocket-bridge` so the local bridge registers session tickets, requires daemon/companion connect messages before adding sockets to a session, and rejects unauthenticated frame messages or bad-token connects before frame routing. `npm run check:pwa-relay-transport-decision` now asserts the authenticated WebSocket bridge evidence while keeping HTTP polling as fallback/diagnostics.
- **Relay signed ticket prototype**: Added `docs/superpowers/plans/2026-07-02-ra-pwa-relay-signed-ticket-prototype.md`, Rust `CompanionRelaySignedSessionTicket`, PWA WebCrypto HMAC helpers, and direct `hmac`/`sha2` remote dependencies. The WebSocket bridge smoke now accepts only `hmac-sha256` signed tickets and rejects unsigned or bad-MAC tickets before connect/frame routing while preserving the existing frame counts.
- **Relay session rotation/reconnect evidence**: Added `docs/superpowers/plans/2026-07-02-ra-pwa-relay-session-rotation-reconnect.md` and extended `npm run smoke:pwa-relay-websocket-bridge` plus `npm run check:pwa-relay-transport-decision` so the local WebSocket bridge rejects expired-ticket connects, reconnects daemon/companion peers through a rotated session id/token, rejects old-token reconnect attempts, and keeps old-session frames isolated from the new endpoint.
...
- **PWA relay visible setup UI**: Added `docs/superpowers/plans/2026-07-02-ra-pwa-relay-visible-setup-ui.md`, a visible PWA `Relay` tab, runtime setup JSON parser/preflight helper, secret-field rejection, and `npm run smoke:pwa-relay-setup-ui` browser evidence. Valid self-hosted setup bundles now render ready state, endpoint, device, session, expiry, key id, and daemon/companion connect JSON while the product default remains `live-loopback`.
- **PWA relay endpoint loop**: Added `docs/superpowers/plans/2026-07-02-ra-pwa-relay-pwa-endpoint-loop.md` and PWA endpoint-loop helpers that derive WebSocket connect URLs from validated runtime setup, authenticate with connect JSON, send live messages as relay frames, and decode incoming relay frames back to live messages. `npm run smoke:pwa-relay-websocket-bridge` now proves the setup-derived PWA loop over the authenticated local WebSocket bridge.
- **Relay daemon transport selection**: Added `docs/superpowers/plans/2026-07-02-ra-pwa-relay-daemon-transport-selection.md` and `ai remote daemon --transport relay --relay-endpoint-url <url>`. Explicit relay daemon startup validates mode/TTL/endpoint, issues setup JSON from the daemon runtime, and still reports the effective approval bridge as `live-loopback` until the relay gate loop has approve/reject evidence.
- **Relay daemon gate bridge**: Added `docs/superpowers/plans/2026-07-02-ra-pwa-relay-daemon-gate-bridge.md` and daemon-side `decide_with_remote_relay_bridge`. The helper sends planned `ApprovalRequest` messages through a relay roundtrip boundary, accepts only `ApprovalResponse` messages back, and reuses the existing nonce, registered-device signature, and context drift validator.
- **Relay session handoff**: Added `docs/superpowers/plans/2026-07-02-ra-pwa-relay-session-handoff.md` to capture the session closeout, focused verification, PR/merge target, and next priority order. The first next slice is binding the relay gate bridge helper to the actual daemon relay runtime loop, followed by PWA Relay browser/operator approve and reject evidence.
...
- **Release follow-up status smoke**: Added `scripts/smoke-release-followup-status.ps1` and npm script `smoke:release-followup-status`. The smoke uses synthetic blocked and ready evidence to verify `show-release-followup-status.ps1` text output, `-Json` summary output, and `-FailOnBlocked` exit-code behavior without depending on host MSI tooling, GitHub secrets, or F-Droid buildserver state. Added `docs/superpowers/plans/2026-07-01-release-followup-status-smoke.md` and linked the smoke from the release runbook/troubleshooting/handoff docs.
- **Release follow-up status command**: Added `scripts/show-release-followup-status.ps1` and npm script `status:release-followup`. The command reads or refreshes combined release follow-up evidence, prints human-readable blocker/next-action status, supports `-Json` for automation, and exits `2` with `-FailOnBlocked` while `closeout.canCloseDocs=false`. Added `docs/superpowers/plans/2026-07-01-release-followup-status-command.md` and linked the command from release docs/runbook/troubleshooting/handoff docs.
...
- **Release follow-up checks**: Confirmed the public `v0.3.2` GitHub Release note already carries a superseded-by-v0.3.3 banner. The public `v0.3.3` release body was empty before the 2026-07-01 follow-up and is now filled without changing the tag or assets.
