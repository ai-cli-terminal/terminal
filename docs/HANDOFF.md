...
`docs/superpowers/plans/2026-07-05-ra-pwa-relay-managed-runtime-operator-setup-approval-response-endpoint-browser-evidence.md`다. Relay/M2는
...
않는다. 다음 작업은 managed approval response daemon bridge evidence로, browser에서
증명한 endpoint path를 daemon approval verification boundary에 붙이는 것이다.
