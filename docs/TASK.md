...
- [~] P3-1-4 바이너리 서명(§29.11 full, R0 이연분) — 1차 substrate: `src/binary_manifest.rs`가 signed `binary-manifest.json` payload를 shared org anchor와 trust manifest로 검증하고, `ai release manifest status/verify`가 active/candidate manifest와 optional artifact hash match를 진단한다. 2차 release asset slice: `ai release manifest create/sign`과 release workflow aggregate job이 CLI/Windows GUI/Android assets를 모아 signing secret 구성 시 `binary-manifest.json` + `binary-manifest.manifest.json` release asset을 업로드한다. 3차 install/update slice: `install.sh`/`install.ps1`가 signed manifest assets를 내려받고 기존 trust-enabled `ai` 또는 `AI_MANIFEST_VERIFIER`로 검증하며, `AI_REQUIRE_SIGNED_MANIFEST=1`에서 fail-closed하고 verified manifest version downgrade를 차단한다. 남은 work: fresh-install strict bootstrap을 배포/운영 문서로 확정하고 실제 signed release evidence로 closeout
