...
| GUI smoke가 `ash.exe child process did not appear within 20s`로 실패 | GUI가 워크스페이스 상태를 영속화해서 직전 실행에서 고른 런타임(예: PowerShell 페인)으로 뜸 | `scripts/smoke-gui.ps1`이 시작 런타임을 항상 명시 주입한다(기본 `ash`, `-Runtime`으로 변경). 구버전 스크립트라면 `AI_TERMINAL_GUI_SMOKE_RUNTIME=ash`를 직접 설정한다. |
| GUI smoke가 `ash integration database evidence did not appear`로 실패 | 스테이징한 `ash.exe`가 `storage` feature 없이 빌드돼 감사 DB(`ai-terminal.db`)를 만들지 않음 | `cargo build --release --bins --features "storage remote trust"`로 다시 빌드해 패키지에 넣는다. 기본 `cargo build --release`는 storage가 빠진다. |
...
Windows MSI가 blocked면 nested
`artifacts\release-followup-preflight\msi-preflight-evidence.json`의
`missing`과 `checks`를 확인한다. `-RunMsiBuild`를 썼을 때
`buildExitCodeZero`, `msiGenerated`, `msiSha256Recorded`가 모두 `true`여야 한다.
