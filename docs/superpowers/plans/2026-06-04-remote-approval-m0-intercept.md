# Remote Approval M0 — 셸 인터셉트 제어점 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** armed 상태에서 위험 명령을 **실행 전** 셸 hook으로 가로채 로컬 위험도 게이트(§30-13)로 통과/차단하는 제어점을 bash·zsh에 in-repo로 착지한다(크립토·데몬·PWA는 M1+).
...
**Tech Stack:** Rust(기존 크레이트), clap 서브커맨드, 기존 `risk`/`policy`/`config`/`shell` 모듈, WSL bash/zsh + PTY e2e.
...
> **NOTE (구현자):** Step 2의 `armed_allows_low_and_medium`·`armed_high_blocks_unless_optin`은 실제 `risk::assess` 점수에 의존한다. Step 4에서 테스트가 빨강이면(등급 가정이 어긋나면) 먼저 `cargo run -- risk "<cmd>"`로 실제 등급을 확인하고 **테스트의 예시 명령을 실제 등급에 맞는 것으로 교체**한 뒤 진행하라(로직이 아니라 픽스처를 맞춘다). Critical 확정 케이스 `rm -rf /`는 안정적이다.

- [ ] **Step 3: 실패 확인**

Run: `wsl.exe -- bash -lc 'source ~/.cargo/env; cd /mnt/d/workspace/terminal-project/terminal; export CARGO_TARGET_DIR=$HOME/targets/ai-terminal; cargo test gate:: 2>&1 | tail -20'`
Expected: 컴파일은 되고 테스트 실행, 등급 가정 불일치 시 일부 FAIL.

- [ ] **Step 4: 등급 픽스처 보정 후 통과 확인**

필요 시 위 NOTE대로 명령 교체. 그 외 로직 변경 없음.
Run: 위와 동일.
Expected: `test result: ok.` (gate 테스트 전부 통과)
...
### Task 2: `gate.rs` — armed 상태 파일 (parse/render + I/O)

**Files:**
- Modify: `src/gate.rs`

- [ ] **Step 1: 실패하는 테스트 + 구현 추가**
...
Run: `wsl.exe -- bash -lc 'source ~/.cargo/env; cd /mnt/d/workspace/terminal-project/terminal; export CARGO_TARGET_DIR=$HOME/targets/ai-terminal; cargo test gate:: 2>&1 | tail -20'`
Expected: 신규 두 테스트 포함 컴파일·통과(구현을 함께 넣었으므로 바로 green이면 OK — TDD red 단계가 짧음).

- [ ] **Step 3: 통과 확인**

Run: 위와 동일. Expected: `test result: ok.`
...
### Task 3: `ai __gate` 내부 커맨드 (exit code 계약 + fail-closed)
...
Run: `wsl.exe -- bash -lc 'source ~/.cargo/env; cd /mnt/d/workspace/terminal-project/terminal; export CARGO_TARGET_DIR=$HOME/targets/ai-terminal; cargo test cli_parses_gate 2>&1 | tail -8'`
Expected: PASS.

수동 동작 확인(armed 아님 → 통과 exit 0; Critical은 armed에서만 차단):

Run: `wsl.exe -- bash -lc 'source ~/.cargo/env; cd /mnt/d/workspace/terminal-project/terminal; export CARGO_TARGET_DIR=$HOME/targets/ai-terminal; cargo run -q -- __gate "rm -rf /" && echo GATE_ALLOW || echo GATE_BLOCK'`
Expected: `GATE_ALLOW` (armed 아니므로 통과). **종료코드는 `&&/||`로만 검증**([[terminal-build-env]]).
...
### Task 4: `ai remote arm/disarm/status` 커맨드
...
- [ ] **Step 4: 빌드·테스트·왕복 동작 확인**

Run: `wsl.exe -- bash -lc 'source ~/.cargo/env; cd /mnt/d/workspace/terminal-project/terminal; export CARGO_TARGET_DIR=$HOME/targets/ai-terminal; cargo test cli_parses_remote 2>&1 | tail -8'`
Expected: PASS (2 tests).

armed 후 게이트가 Critical 차단하는지 e2e 동작 확인(임시 XDG로 격리):

Run: `wsl.exe -- bash -lc 'source ~/.cargo/env; cd /mnt/d/workspace/terminal-project/terminal; export CARGO_TARGET_DIR=$HOME/targets/ai-terminal; export XDG_CONFIG_HOME=$(mktemp -d); cargo run -q -- remote arm; cargo run -q -- __gate "rm -rf /" && echo ALLOW || echo BLOCK; cargo run -q -- remote disarm; rm -rf "$XDG_CONFIG_HOME"'`
Expected: `원격 게이트 armed.` 다음 줄 `BLOCK` (armed에서 Critical 차단).
...
### Task 5: bash hook에 인터셉트(extdebug + 병합 DEBUG trap) 추가
...
Run: `wsl.exe -- bash -lc 'source ~/.cargo/env; cd /mnt/d/workspace/terminal-project/terminal; export CARGO_TARGET_DIR=$HOME/targets/ai-terminal; cargo test shell:: 2>&1 | tail -15'`
Expected: `test result: ok.` — 특히 `generated_hooks_pass_syntax_check`(bash -n)와 신규 `bash_hook_has_armed_intercept` 통과.
...
### Task 6: zsh hook에 인터셉트(ZLE accept-line 위젯) 추가

**Files:**
- Modify: `src/shell.rs` (`ZSH_HOOK` 상수 + 단위 테스트)

- [ ] **Step 1: `ZSH_HOOK`에 위젯 추가**
...
Run: `wsl.exe -- bash -lc 'source ~/.cargo/env; cd /mnt/d/workspace/terminal-project/terminal; export CARGO_TARGET_DIR=$HOME/targets/ai-terminal; cargo test shell:: 2>&1 | tail -15'`
Expected: `test result: ok.` — `generated_hooks_pass_syntax_check`(zsh -n, zsh 설치 시)와 신규 테스트 통과.
...
### Task 7: 대화형 e2e — 생성된 hook이 실제로 차단/통과 (unix)
...
> **NOTE (구현자):** `rm -rf '<dir>'`는 risk 등급이 Critical이 아닐 수 있다(절대경로·시스템경로가 Critical 트리거). e2e의 목적은 "차단 동작" 증명이므로, **armed에서 확실히 Block 되는 명령**을 써야 한다. Task 1 NOTE처럼 `cargo run -- risk "rm -rf '<tmp>/target'"`로 등급을 확인하고, Block 등급(High opt-in 꺼짐 또는 Critical)이 나오는 명령으로 맞춘다. 안 되면 확실한 Critical(`rm -rf /` 대신 부수효과 없는) 대체가 어렵다 → 대상 디렉터리를 `/tmp` 아래 두되 **게이트가 Block하도록 `allow_high=false` + High 트리거(예: `sudo` 포함)** 명령으로 바꾸거나, 테스트용으로 게이트 임계를 명확히 만족시키는 명령을 선택한다. 핵심: 등급은 `risk`로 실측해 픽스처를 맞춘다(로직 변경 금지).

- [ ] **Step 2: e2e 실행(빌드 후)**

Run: `wsl.exe -- bash -lc 'source ~/.cargo/env; cd /mnt/d/workspace/terminal-project/terminal; export CARGO_TARGET_DIR=$HOME/targets/ai-terminal; cargo test --test intercept_e2e -- --test-threads=1 --nocapture 2>&1 | tail -25'`
Expected: `bash_armed_blocks_rm_rf_allows_safe ... ok` (+ zsh ok 또는 skip). 8s 워치독으로 무한 행 없음.
...
### Task 8: 문서 동기화 + 전체 검증 + 통합 커밋/푸시

**Files:**
- Modify: `docs/HISTORY.md`, `docs/TASK.md`

- [ ] **Step 1: HISTORY.md 엔트리 추가**
...
Expected: 모든 `test result: ok.`, clippy 경고/에러 없음.

- [ ] **Step 4: 문서 커밋 + push (사용자 확인 후)**
...
- **Spec coverage**: DoD #1(hook 설치·문법·idempotent) → Task 5/6 + 기존 install 머신 재사용. #2/#3(bash/zsh 대화형 차단) → Task 7. #4(비-armed 무개입) → Task 7 disarmed 경로 + hook의 armed-file stat(Task 5/6 설계). #5(게이트 등급 경계 + fail-closed) → Task 1/3. #6(빌드 green) → Task 8. ✅
- **제외 범위 명시**: 크립토·데몬·소켓 서버·PWA·페어링·TTL/heartbeat/viz·replay/nonce·revoke·TOCTOU = M1+ (Task 본문에 없음, 의도적).
- **타입 일관성**: `GateDecision`(Allow/Block{reason}), `ArmState{allow_high}`, `decide_gate(command, armed, allow_high)`, `armed_path/load_arm_state/arm_at/disarm_at`, CLI `Command::Gate{command:Vec<String>}`·`Command::Remote{action:RemoteAction}` — 전 Task 일관.
- **리스크**: 셸 hook 정확성은 단위 테스트로 완전 검증 불가 → `bash -n`/`zsh -n`(기존 테스트 자동 확장) + Task 7 대화형 e2e + spike 사전 실증으로 3중 방어. risk 등급 가정은 Task 1/7 NOTE로 실측 보정(로직 불변).

## Execution Handoff

계획 저장: `docs/superpowers/plans/2026-06-04-remote-approval-m0-intercept.md`. 사용자가 "구현 진행"을 이미 지시 → **Inline Execution(executing-plans)**으로 이 세션에서 Task 1→8 TDD 실행, Task마다 커밋, Task 8 push 전 확인.
