...
2. **Relay/M2**: local live loopback default를 유지한 상태에서 relay/Tailscale/WebSocket transport를 별도 설계로 착수한다. 2026-07-02 기준 kickoff 문서는 `docs/superpowers/plans/2026-07-02-ra-pwa-relay-m2-transport-kickoff.md`이고, transport catalog/status 기반은 `906da34 feat(remote): catalog companion transports`에 커밋됐다. 다음 slice 문서는 `docs/superpowers/plans/2026-07-02-ra-pwa-relay-session-contract.md`이며, relay frame/session metadata 계약을 먼저 고정한다.
