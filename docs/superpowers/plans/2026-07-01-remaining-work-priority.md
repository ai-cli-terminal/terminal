...
|---|---|---|---|
| P1 local | Private-network relay connection controls | Add explicit private-network connect/disconnect controls and browser connect evidence without changing `live-loopback` default or self-hosted setup behavior | Visible import/status path is complete; managed relay stays deferred |
