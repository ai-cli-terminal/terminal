...
>
> **현재 우선순위(2026-08-01)**: Windows 목표는 `ash.exe`를 Windows Terminal/PowerShell/Git Bash에서 수동 검증하는 모델이 아니라, 사용자가 더블클릭해 여는 **완전한 독립 GUI 터미널 프로그램 `ai-terminal.exe`** 로 전환했다. 기존 Windows native `ash.exe` S1~S7 구현은 GUI 앱 내부 PTY/ConPTY child runtime으로 재사용한다. 정본: `docs/superpowers/specs/2026-06-27-windows-gui-terminal-pivot-design.md`. GUI scaffold/PTY bridge/packaging/portable smoke/NSIS install-run-uninstall smoke와 GUI 내부 AI routing/safety gate/storage-audit evidence가 green이며 MSI 실제 build/path/SHA256 evidence도 ready다. 2026-08-01 재검증에서 F-Droid Docker-local build evidence도 ready로 확인했다. **2026-08-15에는 Android signing secret 4개가 등록되어 combined check가 `closeout.canCloseDocs=true`, `blockedItems=[]`를 냈다 — 외부 릴리스 follow-up blocker는 0이다.** 남은 Windows 수동 후속은 필요 시 literal Explorer 더블클릭 영상뿐이고, 남은 즉시 우선순위는 Android 실기기 openai 실 응답 검증이다.
...
| Windows GUI `ai-terminal.exe` | [x] | 독립 Tauri/xterm GUI, bundled `ash.exe` PTY bridge, terminal UX, AI/safety/storage/audit, portable/NSIS/MSI evidence green, release follow-up preflight/runbook | 필요 시 수동 literal Explorer 더블클릭 영상만 후속 |
| Android 로컬 터미널 | [~] release follow-up closeout 완료(2026-08-15) | Kotlin/Compose skeleton, worker thread + stream/cancel JVM contract, Rust `MobileShell` pure core boundary, JNI bridge + instrumentation smoke, app-private workspace/cwd boundary, document import/export + safe reader metadata, UTF-8 boundary-safe previews, `Export Last`, selected-file shellcore helpers, full-ABI JNI packaging CI, shellcore-only MVP와 PM-3E 외부 명령 전략 결정, PM-3F Termux opt-in bridge design, T0/T1 Termux real-device smoke, shared staging diagnostics, manual import/open/export/helper real-device smoke green, F-Droid Docker-local build evidence ready, GitHub Android signing secrets 4개 등록 완료 | 남은 것은 실기기 openai 실 응답 검증(device-only) |
...
- [x] Windows packaging/smoke: portable package/zip 생성 완료(`ai-terminal.exe` + `ash.exe` + `ai.exe` + checksum + GUI smoke script), 자동 GUI smoke(창/`ash.exe` child/prompt·input·output screenshot+transcript/resize screenshot/frontend UX evidence/AI routing+safety gate+storage/audit evidence/Ctrl-C recovery screenshot/Ctrl-D exit screenshot/cleanup) green, NSIS installer artifact + SHA256 생성 완료, installer silent install/installed GUI smoke/uninstall cleanup green. v0.3.3 GitHub Release body도 Windows GUI/CLI asset 구분과 checksum 안내로 보강했다. `scripts/smoke-release-followup-preflight.ps1`는 MSI/Android signing/F-Droid buildserver blocker를 통합 JSON으로 기록하고, MSI `-RunMsiBuild` generated MSI/hash evidence, Android signing workflow secret references, F-Droid supplied evidence의 expected app id/version/result/artifact marker도 확인한다. release follow-up 문서 closeout은 `closeout.canCloseDocs=true`와 `closeout.blockedItems=[]`가 evidence에 기록될 때만 진행한다. `npm run status:release-followup`는 이 evidence를 사람이 읽는 blocker/next action 보고서로 요약하고, `npm run smoke:release-followup-status`는 synthetic evidence로 status command 계약을 검증한다. `npm run check:release-followup`는 status smoke/preflight/status summary를 한 번에 실행한다. `docs/releases/release-followup-runbook.md`는 외부 host에서 이를 닫는 절차를 고정한다. 2026-07-24 MSI `-RunMsiBuild` evidence ready
...
**Windows 완료 기준**: `ai-terminal.exe`가 자체 GUI 창에서 PTY/ConPTY 기반 `ash` 세션을 실행하고, 외부 Windows Terminal/PowerShell/Git Bash 창 없이 terminal UX와 safety/AI/storage 경로가 통과해야 한다. 이 자동 완료 기준은 2026-06-28 portable/NSIS smoke에서 충족됐다. 2026-06-30에 MSI preflight blocked evidence와 Windows ShellOpen launch evidence도 확보했고, 2026-07-24에는 Windows-native Rust/MSVC + Tauri-managed WiX 3.14로 MSI를 실제 생성해 `msi.checks.buildExitCodeZero`, `msi.checks.msiGenerated`, `msi.checks.msiSha256Recorded`를 모두 `true`로 만들었다. 2026-08-01 재검증 MSI SHA256은 `ad0dad98d879b35ab75453f98cf5a08f6bd5f1ebcfd9441880415e87d710c640`이다. 필요 시 문자 그대로의 수동 Explorer 더블클릭 영상 evidence만 별도 수동 후속이다.

### PM-3 — Android 로컬 터미널 스파이크
> 현재 상태: Windows GUI 터미널 자동 완료 기준 이후 Android PM-3 local UX와 direct APK/F-Droid release 입력 대부분을 닫았다. 2026-07-06 기준 imported document reader metadata, UTF-8 boundary-safe previews, `Export Last`, selected-file shellcore helpers, Termux shared staging diagnostics, and real-device manual smoke capture가 green이다. `:app:verifyFdroidReleaseInputs`, fdroid metadata lint/rewritemeta, throwaway-keystore GitHub signing preflight, fdroid activation dry-run도 green이다. 2026-07-24에는 공식 fdroidserver 이미지 기반 Docker local mode에서 `dev.aiterminal.android` `0.5.0`/`500`을 실제 빌드하고 unsigned APK의 4 ABI와 SHA256을 검증해 combined preflight의 `fdroidBuild.status=ready`를 확보했다. 이는 공식 F-Droid VM buildserver 실행은 아니다. 2026-08-15에 실제 GitHub Android signing secret 4개가 등록되어 combined evidence가 `closeout.canCloseDocs=true`, `closeout.blockedItems=[]`를 냈고 이 문서를 완료 처리했다(secret 값은 읽거나 저장하지 않았다). 실 서명 아티팩트 검증은 다음 태그 push 때 `release.yml` android 잡에서 확인한다. 남은 Android 작업은 실기기 `SM-F956N` openai 실 응답 검증뿐이다.
...
- [x] Termux shared staging diagnostics: `app-write` read/write probe와 `helper-marker` transcript 진단 분리, marker missing 시 external commands fail-closed 유지
...
- [x] UTF-8 preview boundary polish: preview byte limit이 multi-byte 문자 중간에서 끊겨도 유효한 UTF-8 prefix를 truncated text로 표시하고, boundary 이전 invalid UTF-8은 계속 binary/non-UTF-8로 처리
- [x] Imported workspace file export UX: `Export Last`로 마지막 import 파일을 SAF destination에 복사, workspace canonicalization 재사용, app-private path 자동 노출 금지
- [x] Selected-file shellcore helper: `List Files`/`Find Last`가 workspace-relative `ls` 명령을 input에 준비, 자동 실행/절대경로 노출/raw read 미수행
- [x] Android real-device smoke capture: `SM-F956N`에서 import/open/export, `List Files`/`Find Last`, Termux staging `app-write`/`helper-marker`, `external / staging` 상태를 manual UI와 instrumentation evidence로 확인
...
- [x] P3-1-3 스킬 서명 + 조직 레지스트리(§26.6, 외부 기본 비활성, update/revoke·감사) — signed `org-registry.json` + manifest를 shared org anchor로 검증하고, registry/manifest가 있으면 invalid set은 fail-closed. `ai skill`은 registry active+SHA-256 일치 스킬만 표시하고 revoked/unsigned/modified skill은 숨긴다. `ai skill registry status`는 path/anchor/subject/active-invalid diagnostics와 entry counts를 제공한다. `ai skill registry update/revoke`는 signed snapshot을 검증 후 active registry로 설치하고 storage builds는 update/revoke/enforcement audit metadata를 기록한다. user-config external skills는 기본 숨김이며 `ai skill enable/disable/enabled`로 명시 활성화한다. signed `policy.d`의 `[skills].external_sources`는 `user-enabled`/`registry-only`/`disabled`로 외부 스킬 enable 경계를 조직 정책에 연결한다. `ai skill enable <name>`은 기본적으로 정확한 스킬 이름 확인을 요구하며 자동화는 `--yes`를 명시한다.
- [~] P3-1-4 바이너리 서명(§29.11 full, R0 이연분) — 1차 substrate: `src/binary_manifest.rs`가 signed `binary-manifest.json` payload를 shared org anchor와 trust manifest로 검증하고, `ai release manifest status/verify`가 active/candidate manifest와 optional artifact hash match를 진단한다. 2차 release asset slice: `ai release manifest create/sign`과 release workflow aggregate job이 CLI/Windows GUI/Android assets를 모아 signing secret 구성 시 `binary-manifest.json` + `binary-manifest.manifest.json` release asset을 업로드한다. 3차 install/update slice: `install.sh`/`install.ps1`가 signed manifest assets를 내려받고 기존 trust-enabled `ai` 또는 `AI_MANIFEST_VERIFIER`로 검증하며, `AI_REQUIRE_SIGNED_MANIFEST=1`에서 fail-closed하고 verified manifest version downgrade를 차단한다. 4차 bootstrap 문서 slice: `docs/releases/signed-binary-manifest-bootstrap-runbook.md`가 fresh strict install의 선택 경로를 MDM/golden image/internal package manager로 사전 배포된 managed verifier bundle로 고정하고, `npm run check:release-manifest-bootstrap`가 이 경계를 감시한다. 5차 evidence gate: `npm run check:release-manifest-bootstrap-evidence`가 외부 조직 배포 evidence 없이는 blocked로 남고, verifier preinstall/anchor/strict env/install success/self-verification 금지/downgrade guard를 검증한다. 남은 work: 실제 signed release evidence와 조직 verifier bundle 배포 evidence로 closeout
