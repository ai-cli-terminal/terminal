...
- **완료 기준(§31.7)**: 예산 100% 시 원격 AI 차단, 모든 AI 요청 usage 기록, 부정확 비용 estimated 표기. `--features storage` 통합테스트.
...
- **완료 기준(§31.8)**: `.env` 원격 컨텍스트 제외 검증. **보안 민감 → SECADMIN 범위.**
...
- **완료 기준(§31.1/§31.10)**: bash에서 `cd` 후 컨텍스트 cwd·git_branch 갱신, hook 실패가 셸 비중단. `bash -n` + WSL e2e.
...
- **완료 기준(§31.1)**: hook 미가용 시 wrapper로 동등 컨텍스트 수집, 일반 명령 투명 통과.
...
- **완료 기준(§31.5 UX)**: 장기 명령 출력 라이브 표시, 실행 중 중단 동작.
