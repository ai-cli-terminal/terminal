...
2. **Relay/M2**: local live loopback default를 유지한 상태에서 relay/Tailscale/WebSocket transport를 별도 설계로 착수한다. 2026-07-02 기준 kickoff 문서는 `docs/superpowers/plans/2026-07-02-ra-pwa-relay-m2-transport-kickoff.md`이고, transport catalog/status 기반은 `906da34 feat(remote): catalog companion transports`, relay frame/session metadata 계약은 `770e754 feat(remote): define relay session frame`, local relay harness는 `98910aa feat(remote): add relay loopback harness`, endpoint adapter는 `2f90cc2 feat(remote): add relay endpoint adapter`, PWA frame parity는 `5f64aa7 feat(pwa): add relay frame parity`, PWA endpoint helper는 `ff28ebd feat(pwa): add relay endpoint helper`에 커밋됐다. PWA relay exchange smoke 문서는 `docs/superpowers/plans/2026-07-02-ra-pwa-relay-exchange-smoke.md`이며, browser smoke 전 PWA endpoint request/response exchange를 JSON frame 왕복으로 고정한다. 다음 follow-up은 relay threat model/envelope mapping과 browser relay parity smoke다.
