...
`docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-client-key-agreement-runtime-smoke.md`다. Relay/M2는
...
encrypted frame envelope smoke와 session-bound client-held key agreement smoke도
완료됐지만 implementationCanStart=false다. explicit self-hosted relay readiness는
green이며 다음 로컬 작업은 Managed relay metadata minimization review다.
