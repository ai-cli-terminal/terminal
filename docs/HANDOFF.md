...
`docs/superpowers/plans/2026-07-04-ra-pwa-relay-private-network-connection-controls.md`다. Relay/M2는
...
Private-network relay approval flow evidence다.
...
2. **Relay/M2**: `live-loopback`은 계속 product default다. Explicit self-hosted relay readiness는 `docs/superpowers/plans/2026-07-04-ra-pwa-relay-failure-mode-evidence.md`, `npm run smoke:pwa-relay-service-artifact`, `npm run smoke:pwa-relay-websocket-bridge`, `npm run check:pwa-relay-hosted-readiness` 기준으로 green이다. `docs/superpowers/plans/2026-07-04-ra-pwa-relay-private-network-connection-controls.md`와 `npm run smoke:pwa-relay-private-network-connection-controls`가 PWA Relay tab에 별도 private-network connect/disconnect controls와 setup-derived browser WebSocket connect evidence를 추가하고 self-hosted setup/connect behavior를 유지했다. Daemon gate bridge 문서는 `docs/superpowers/plans/2026-07-02-ra-pwa-relay-daemon-gate-bridge.md`이며, `decide_with_remote_relay_bridge`가 relay roundtrip으로 받은 approval response를 기존 nonce/signature/context 검증 경계에 접는다. HTTP polling은 fallback/diagnostics 후보로 유지한다. 다음 follow-up은 Private-network relay approval flow evidence다.

3. **다음 세션 시작점**: 최신 Relay production-readiness 문서는
   `docs/superpowers/plans/2026-07-04-ra-pwa-relay-private-network-connection-controls.md`다. 첫 작업은
   Private-network relay approval flow evidence를 진행하는 것이다.
