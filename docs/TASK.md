...
> 현재 상태: Windows GUI 터미널 자동 완료 기준 이후 Android PM-3 UX와 direct APK/F-Droid release 입력 대부분을 닫았다. 2026-07-01 기준 `:app:verifyFdroidReleaseInputs`, fdroid metadata lint/rewritemeta, throwaway-keystore GitHub signing preflight, fdroid activation dry-run은 green이다. `smoke-release-followup-preflight.ps1`는 `.github/workflows/release.yml`의 Android signing secret references를 확인하고, 실제 GitHub signing secret names와 F-Droid build/buildserver evidence path가 아직 없어 blocked를 기록한다. supplied F-Droid evidence는 `dev.aiterminal.android`/`0.3.3`/`303`/success/artifact marker를 포함해야 ready가 된다. release follow-up closeout은 combined evidence의 `closeout.canCloseDocs=true`와 `closeout.blockedItems=[]`까지 확인한 뒤 문서 완료 처리한다. 다음 Android 후속은 `docs/releases/release-followup-runbook.md`를 따라 실제 GitHub signing secrets 등록/검증과 fdroidserver build/buildserver 검증을 닫는 것이다.
