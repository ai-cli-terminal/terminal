...
> **정본(source of truth)**: `../../document/` (설계 v3.3, "MVP spec finalized — ready to build").
...
| **RESEARCH** 연구자 | 반복 실험 자동화, 재현성 | 자동화 스크립트, 사용량·토큰 추적, 작업 로그 |
| **SECADMIN** 보안 관리자 | 정책 강제, 마스킹, 감사 | 정책 프로파일, Secret/PII 마스킹, 감사 이벤트 |

## 5. 범위

### 5.1 MVP+ 포함 (Phase 1, §25.1 · 계획서 §1.4)

PTY+bash/zsh 호환 · `ai "..."` 인라인/자연어 변환/설명 · 실행 전 확인+Critical 차단 · **Ctrl+C AI 취소 & Graceful Recovery** · **AI 타임아웃** · **Secret/PII 마스킹** · **Token Window 기본** · **`ai preview`+diff** · **Context Consistency 기본** · **Policy Profile(balanced/paranoid)** · **alias 충돌 감지** · 에러 분석/세션 히스토리/감사 로그/사용량·토큰 추적.
...
핵심 도메인 7개: 1) Terminal Core 2) AI Service 3) Security & Policy 4) Context Consistency Manager 5) Execution Guardrails Engine 6) Storage & Audit 7) Subsystems(Skill§26/MCP§27/Remote§28). **MVP는 1~6 구현, 7은 Phase 2+.**
