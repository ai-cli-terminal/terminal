...
- 외부 명령 전략은 `shellcore-only` MVP를 기준선으로 둔다. 다음 후보는 Termux-compatible opt-in bridge이고, bundled userland는 보류한다.
...
첫 결정은 `shellcore-only`다. PM-3E 비교 결과는 `2026-06-24-android-external-command-strategy.md`가 정본이다.

| 선택지 | 이번 판단 |
|---|---|
| `shellcore-only` | 채택. Android UI와 Rust core bridge를 가장 작게 검증한다. |
| Termux-compatible | 다음 spike. 사용자가 설치한 compatible runtime을 direct PATH가 아니라 explicit opt-in bridge로 연결한다. |
| bundled minimal userland | 보류. 배포 크기, 4 ABI packaging, CVE update, licensing, binary provenance 책임이 생긴다. |
