...
> **플랫폼 정본**: `2026-06-23-platform-target-matrix-design.md` — 모바일도 PWA 승인 companion가 아니라 **로컬 터미널** 목표를 가진다.
...
### 0.1 정체성
Nushell 계열 **독립 구조화 셸** + AI/안전이 1급 빌트인. bash/zsh를 감싸지 않는다 — **이 셸이 곧 셸**이며, 외부 바이너리는 플랫폼 adapter를 통해 직접 spawn한다. 명령은 텍스트가 아니라 **구조화 데이터(레코드/테이블)**를 파이프라인으로 흘린다. 같은 `shellcore`를 Linux/WSL/Windows와 모바일 로컬 터미널에서 공유한다.
...
| S4 — 플랫폼/고급 | Windows shell bridge · Android 로컬 터미널 · iOS research · 원격 승인 companion(기존 RA 재배치) · 잡 컨트롤 · 스킬/MCP · 플러그인 |
...
- 기존 Phase 3 로드맵(R0/RA/P3, "bash 위 보조" 전제)은 본 피벗으로 **재정렬 대상**: R0 릴리즈 도구는 새 `ash` 바이너리에 재사용, RA/원격승인은 S4 companion 기능으로 재배치, P3-1~3(조직정책/감사/MCP)은 셸 위 기능으로 후속 재평가. 플랫폼별 목표와 모바일 로컬 터미널 전환은 `2026-06-23-platform-target-matrix-design.md`를 따른다.
