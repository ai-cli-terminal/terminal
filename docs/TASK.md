...
**Windows 완료 기준**: `ai-terminal.exe`가 자체 GUI 창에서 PTY/ConPTY 기반 `ash` 세션을 실행하고, 외부 Windows Terminal/PowerShell/Git Bash 창 없이 terminal UX와 safety/AI/storage 경로가 통과해야 한다. 이 자동 완료 기준은 2026-06-28 portable/NSIS smoke에서 충족됐다. 2026-06-30에 MSI preflight blocked evidence와 Windows ShellOpen launch evidence도 확보했고, 2026-07-24에는 Windows-native Rust/MSVC + Tauri-managed WiX 3.14로 MSI를 실제 생성해 `msi.checks.buildExitCodeZero`, `msi.checks.msiGenerated`, `msi.checks.msiSha256Recorded`를 모두 `true`로 만들었다. 필요 시 문자 그대로의 수동 Explorer 더블클릭 영상 evidence만 별도 수동 후속이다.

### PM-3 — Android 로컬 터미널 스파이크
> 현재 상태: Windows GUI 터미널 자동 완료 기준 이후 Android PM-3 local UX와 direct APK/F-Droid release 입력 대부분을 닫았다. 2026-07-06 기준 imported document reader metadata, UTF-8 boundary-safe previews, `Export Last`, selected-file shellcore helpers, Termux shared staging diagnostics, and real-device manual smoke capture가 green이다. `:app:verifyFdroidReleaseInputs`, fdroid metadata lint/rewritemeta, throwaway-keystore GitHub signing preflight, fdroid activation dry-run도 green이다. `smoke-release-followup-preflight.ps1`는 `.github/workflows/release.yml`의 Android signing secret references를 확인하고, 실제 GitHub signing secret names와 F-Droid build/buildserver evidence path가 아직 없어 blocked를 기록한다. supplied F-Droid evidence는 `dev.aiterminal.android`/`0.5.0`/`500`/success/artifact marker를 포함해야 ready가 된다. release follow-up closeout은 combined evidence의 `closeout.canCloseDocs=true`와 `closeout.blockedItems=[]`까지 확인한 뒤 문서 완료 처리한다. 다음 Android release 후속은 `docs/releases/release-followup-runbook.md`를 따라 실제 GitHub signing secrets 등록/검증과 fdroidserver build/buildserver 검증을 닫는 것이다.
