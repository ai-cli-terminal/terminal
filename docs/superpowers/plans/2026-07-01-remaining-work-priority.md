...
|---|---|---|---|
| P1 local | Managed relay runtime implementation plan | Define the managed runtime service boundary, implementation phases, exposure gates, and regression checks while preserving payload-blind, public-verifier, quota, support, and billing/abuse boundaries | Managed runtime readiness evidence is complete and gate is green; `selectedRuntime` remains deferred until the plan explicitly changes exposure |
...
redaction/access review evidence, billing/abuse boundary review도 완료됐다.
가장 높은 가치의 다음 release 작업은 외부 환경에서 runbook을 실행하는
**Windows MSI 재검토**와 **Android signing/buildserver evidence**다. 현재 개발 host에서
바로 진행 가능한 다음 로컬 작업은 **Managed relay runtime implementation plan**이다.
