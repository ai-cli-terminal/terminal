...
|---|---|---|---|
| P1 local | Managed relay client key agreement runtime smoke | Prove session-specific client-held payload keys can be derived or transported across daemon/companion boundaries without exposing them to managed relay routing | Payload-blind frame envelope is complete; managed relay remains deferred |
