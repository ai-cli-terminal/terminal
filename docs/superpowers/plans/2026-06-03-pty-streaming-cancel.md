# W2 PTY 출력 스트리밍 + Ctrl+C 중단 구현 계획

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**목표:** `PtyExecutor`가 PTY 출력을 청크 단위로 즉시 `OutputSink`에 스트리밍하고, CLI에서 Ctrl+C로 실행 중인 자식을 중단한다.
...
**기술 스택:** Rust, portable-pty, tokio(`mpsc`/`signal`/current-thread 런타임 — 기존 dep).
...
### 작업 1: `run_in_pty_streaming` (`src/pty.rs`)

**Files:** Modify `src/pty.rs`(함수 추가 + `#[cfg(all(test, unix))]` 테스트)

- [ ] **단계 1: 실패 테스트 작성** — `src/pty.rs`의 `mod tests`에 추가:
...
실행: `wsl.exe -- bash -c 'source ~/.cargo/env; cd /mnt/d/workspace/terminal-project/terminal; export CARGO_TARGET_DIR=$HOME/targets/ai-terminal; cargo test --lib pty 2>&1 | tail -15'`
기대: 컴파일 에러 — `run_in_pty_streaming` 미정의.

- [ ] **단계 3: 구현** — `run_in_pty` 함수 정의 바로 아래(약 50행)에 추가:
...
실행: `wsl.exe -- bash -c 'source ~/.cargo/env; cd /mnt/d/workspace/terminal-project/terminal; export CARGO_TARGET_DIR=$HOME/targets/ai-terminal; cargo test --lib pty 2>&1 | grep -E "test result|error\["'`
기대: `test result: ok` — 신규 2개 포함 모두 통과.
...
### 작업 2: `PtyExecutor::run` 스트리밍 교체 (`src/pipeline.rs`)
...
- [ ] **단계 2: 빌드·clippy·fmt·테스트(기본 + storage)**
...
```
기대: clippy/fmt clean, 모든 `test result: ok`(기존 pipeline 테스트는 `MockExecutor` 사용이라 무영향).
...
```
기대: 출력에 `onetwo` 포함, 마지막 줄 `NONZERO`(exit 4 → 비0). (`ai exec`가 Allow 등급 명령은 확인 없이 실행.)

- [ ] **단계 2: e2e — Ctrl+C 중단(자식 즉시 종료)**
...
```
기대: `child-ended-promptly`가 (워치독 5초 전에) 빠르게 출력 → SIGINT가 자식을 중단함. `WATCHDOG_KILLED`가 보이면 중단 실패이므로 STOP·보고. (exit 130 정확값은 `$?` 측정 불가라 행동으로 갈음.)

- [ ] **단계 3: `docs/TASK.md` 갱신**

`### W2 PTY 터미널 코어` 아래 항목 중 다음 줄:
```
