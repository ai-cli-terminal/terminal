...
|---|---|---|---|
| P1 local | Production relay service artifact and deploy recipe | repo contains a production-oriented self-hosted relay service entrypoint or package shape, deploy recipe, health endpoint contract, secret/config surface, and local smoke that proves ticket registration + daemon/companion `wss://`-compatible routing contract without exposing payload/secrets | `live-loopback` remains default; verifier-key distribution/signing, payload confidentiality/trust decision, hosted observability, and hosted failure evidence still gate user-selectable relay |
...
hosted/WSS readiness gate, daemon WSS relay runtime support는 완료됐다.
가장 높은 가치의 다음 release 작업은 외부 환경에서 runbook을 실행하는
**Windows MSI 재검토**와 **Android signing/buildserver evidence**다. 현재 개발 host에서
바로 진행 가능한 다음 로컬 작업은 **Production relay service artifact and deploy recipe**다.
