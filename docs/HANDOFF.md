...
`docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-support-redaction-access-review-evidence.md`다. Relay/M2는
...
support redaction/access review evidence까지 닫혔지만
implementationCanStart=false다. explicit self-hosted relay readiness는 green이며
다음 로컬 작업은 Managed relay billing/abuse boundary review다.
...
2. **Relay/M2**: `live-loopback`은 계속 product default다. Explicit self-hosted relay readiness는 `docs/superpowers/plans/2026-07-04-ra-pwa-relay-failure-mode-evidence.md`, `npm run smoke:pwa-relay-service-artifact`, `npm run smoke:pwa-relay-websocket-bridge`, `npm run check:pwa-relay-hosted-readiness` 기준으로 green이다. `docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-control-plane-contract.md`와 `npm run check:pwa-relay-managed-control-plane-contract`가 managed relay control-plane ownership, tenant/session boundary, operator-visible state, audit constraints를 기록했고, `docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-abuse-retention-policy.md`와 `npm run check:pwa-relay-managed-abuse-retention-policy`가 abuse handling, retention/deletion, support workflow constraints를 기록했다. `docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-payload-confidentiality-plan.md`와 `npm run check:pwa-relay-managed-payload-confidentiality-plan`은 managed relay가 payload-blind여야 하며 explicit operator trust만으로는 충분하지 않다는 결정을 기록한다. `docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-verifier-key-operations-policy.md`와 `npm run check:pwa-relay-managed-verifier-key-operations-policy`는 public verifier key only, private signing key exclusion, rotation overlap, revocation fail-closed, key id/version audit boundary를 기록한다. `docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-billing-quota-policy.md`와 `npm run check:pwa-relay-managed-billing-quota-policy`는 tenant-scoped quotas, metered usage dimensions, fail-closed quota enforcement, aggregate-only usage visibility, and billing records without payloads or secrets를 기록한다. `docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-runtime-readiness-gate.md`와 `npm run check:pwa-relay-managed-runtime-readiness-gate`는 managed runtime blocker audit, minimum green evidence, and implementationCanStart=false gate decision을 기록한다. `docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-payload-blind-frame-encryption-spike.md`와 `npm run check:pwa-relay-managed-payload-blind-frame-encryption-spike`는 client-held AES-GCM encrypted frame envelope, route-visible byte-count-only boundary, wrong-key/tamper fail-closed behavior를 기록한다. `docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-client-key-agreement-runtime-smoke.md`와 `npm run check:pwa-relay-managed-client-key-agreement-runtime-smoke`는 X25519 + HKDF-SHA-256 session-bound payload key derivation, public-route-metadata-only fail-closed behavior, wrong-session decrypt failure를 기록한다. `docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-metadata-minimization-review.md`와 `npm run check:pwa-relay-managed-metadata-minimization-review`는 route/control-plane/billing/support/audit metadata allowlists, aggregate-only billing usage, hashed support/audit identifiers, raw ciphertext/key/secret/payload exclusion을 기록한다. `docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-public-verifier-key-registry-runtime-smoke.md`와 `npm run check:pwa-relay-managed-public-verifier-key-registry-runtime-smoke`는 tenant/key-id/key-version public verifier lookup, public-key-only Ed25519 ticket verification, missing/revoked/tampered fail-closed behavior, private signing-key/HMAC secret exclusion을 기록한다. `docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-revocation-and-rotation-propagation-smoke.md`와 `npm run check:pwa-relay-managed-revocation-and-rotation-propagation-smoke`는 active/rotating overlap, retiring/revoked fail-closed propagation, post-rotation active key verification, tenant/key/snapshot audit metadata를 기록한다. `docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-tenant-session-registration-quota-smoke.md`와 `npm run check:pwa-relay-managed-tenant-session-registration-quota-smoke`는 tenant registration quota preflight, exhausted/window-not-effective fail-closed behavior, payload/secret-free quota audit, billing/abuse delta separation을 기록한다. `docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-active-session-and-byte-quota-smoke.md`와 `npm run check:pwa-relay-managed-active-session-and-byte-quota-smoke`는 tenant/daemon active-session ceilings, frame/byte quota fail-closed behavior, payload-free audit, billing/abuse delta separation을 기록한다. `docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-tenant-aggregate-usage-export-smoke.md`와 `npm run check:pwa-relay-managed-tenant-aggregate-usage-export-smoke`는 tenant aggregate usage export, payload/secret exclusion, support aggregate visibility, billing/abuse section separation을 기록한다. `docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-support-redaction-access-review-evidence.md`와 `npm run check:pwa-relay-managed-support-redaction-access-review-evidence`는 redacted support view, hashed identifiers, tenant-admin approval, time-bounded support access, raw identifier rejection을 기록한다. Daemon gate bridge 문서는 `docs/superpowers/plans/2026-07-02-ra-pwa-relay-daemon-gate-bridge.md`이며, `decide_with_remote_relay_bridge`가 relay roundtrip으로 받은 approval response를 기존 nonce/signature/context 검증 경계에 접는다. HTTP polling은 fallback/diagnostics 후보로 유지한다. 다음 follow-up은 Managed relay billing/abuse boundary review다.

3. **다음 세션 시작점**: 최신 Relay production-readiness 문서는
   `docs/superpowers/plans/2026-07-04-ra-pwa-relay-managed-support-redaction-access-review-evidence.md`다. 첫 작업은
   Managed relay billing/abuse boundary review를 진행하는 것이다.
