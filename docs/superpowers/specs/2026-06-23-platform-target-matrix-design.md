...
| Git Bash/MSYS on Windows | P1 | Windows 위 POSIX 호환 프로파일 | MSYS path conversion과 POSIX toolchain을 명시적으로 다룸 | `ash.exe` native 모드와 MSYS bridge 모드 분리 | Windows 설치 후 optional profile | 현재는 주의사항만 있음 |
...
- Git Bash/MSYS를 Windows native와 섞어 암묵 처리하지 않는다. 별도 profile로 둔다.
