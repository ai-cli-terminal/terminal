...
| Android | P1 | **모바일 로컬 터미널 1차 타깃** | 기기 안에서 `ash` UI + 로컬 파일/프로세스/패키지 환경을 제공 | Rust core + Android UI + worker. MVP는 shellcore-only, 다음 후보는 Termux-compatible opt-in bridge, bundled userland는 보류 | APK/F-Droid 우선, Play Store는 정책 검토 후 | Rust `MobileShell` JNI bridge, workspace, import/export, stream/cancel, ABI CI 진행 |
...
4. Android 스파이크: Rust `shellcore` 호출, terminal UI 입력/출력, worker, local file access를 검증하고, Termux-compatible opt-in bridge design spike를 진행한다.
